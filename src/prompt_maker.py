...
def generate_tree_output(directory="."):
    try:
        result = subprocess.run(
            ["tree", "-L", "2", directory], capture_output=True, text=True
        )
        return result.stdout
    except FileNotFoundError:
        return (
            "<Error: 'tree' command not found. Please install it to use this feature.>"
        )


if __name__ == "__main__":
    directory = (
        input("Enter the directory to scan (default is current directory): ") or "."
    )
    ...
    with open(output_file, "w", encoding="utf-8") as out_file:
        out_file.write(
            f"hamishburke@hamishburke ~/D/C/W/R/src (main)> tree -L 2\n{tree_output}\n"
        )
    ...
    copy_to_clipboard = (
        input("Do you want to copy the output to the clipboard? (y/n): ")
        .strip()
        .lower()
    )
